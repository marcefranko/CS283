...
void word_print(char *str){
    ...
    for (int i = 0; i < len; i++) {
        ...
        if (!word_start) {
            if (str[i] != ' ') { // Not a space, start a new word
                wc++;
                word_start = true;
                wlen = 0;
        ...
        if (word_start) {
            if (str[i] != ' ') { // Current character is not a space
                printf("%c", str[i]);
                wlen++;
            }
        }
        ...
        if ((word_start && str[i] == ' ') || (i == last_char_idx)) {
            if (wlen > 0) { // Only print word length if there was a word
                printf(" (%d)\n", wlen);
                word_start = false;
                wlen = 0; // Reset for the next word
            }
        }
    }
}
